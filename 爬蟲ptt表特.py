    ...
    if img_urls:
        try:
            ...
            for img_url in img_urls:
                ...
                #若為m.開頭，則取代為i.，若不含i.則補上
                #若網址非以.jpg結尾也一樣補上.jpg
...
#1.當 Python 檔案（模組、module）被引用的時候，檔案內的每一行都會被 Python 直譯器讀取並執行
#2.Python 直譯器執行程式碼時，有一些內建、隱含的變數，__name__就是其中之一，其意義是「模組名稱」。
#如果該檔案是被引用，其值會是模組名稱；但若該檔案是(透過命令列)直接執行，其值會是 __main__；。
#所以回到上面的問題：要怎麼讓檔案在被引用時，不該執行的程式碼不被執行？當然就是靠 __name__ == '__main__'做判斷！
#參考資料:http://blog.castman.net/%E6%95%99%E5%AD%B8/2018/01/27/python-name-main.html
